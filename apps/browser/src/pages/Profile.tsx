...
const Profile = () => {
  ...
  useEffect(() => {
    ...
    // Fetch member join date from Supabase auth.users metadata or profile table
    const fetchMemberSince = async () => {
      try {
        // Prefer auth user created_at
        const { data: authData, error: authError } = await supabase.auth.getUser();
        if (!authError && authData?.user?.created_at) {
          ...
          // Newbie if account age < 7 days
          const ageMs = Date.now() - created.getTime();
          setIsNewbie(ageMs < 7 * 24 * 60 * 60 * 1000);
          return;
        }

        // Fallback: use created_at from user-profile-get
        if (user?.id) {
        ...
        console.error('Failed to fetch member since date', e);
      }
    };
  ...
  // Load profile stats from Supabase RPCs
    ...
    (async () => {
      try {
        ...
        const [s, h, rp] = await Promise.all([
          ...
          fetchUserGameHistory(user.id, 50),
          fetchUserRoyalePlacements(user.id)
        ]);
        if (!cancelled) {
          setStats(s);
          setHistory(h);
  ...
  // Removed friend suggestions and friends list per request
                  ...
                  <div className="flex-1 w-full">
                    <div className="flex flex-col md:flex-row md:items-center gap-3 mb-3">
                      <div className="w-full flex items-center px-4 py-3 rounded-lg bg-primary/10 text-primary border border-primary/20">
                        <h1 className="text-2xl font-semibold leading-none">{user.username}</h1>
                      </div>
                    </div>
                ...
                <CardContent className="p-6 pt-1">
                  <div className="space-y-3">
                    ...
                    <Button
                      ...
                      className="w-full justify-start"
                    >
                      <ArrowDownCircle className="w-4 h-4 mr-2 text-primary" />
                      Withdraw
                    </Button>
            ...
            <Card className="bg-card border-border">
              ...
              <CardContent className="p-6 pt-0">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5 min-h-[15rem]">
                  {pagedGameHistory.map((game) => (
                    ...
                    >
                      ...
                      <div className="flex justify-between items-center text-[13px] leading-5">
                        <span className="text-muted-foreground">{('dateLabel' in game) ? (game as any).dateLabel : game.date}</span>
                        <div className="flex items-center gap-3">
                          ...
                          <span className={`${game.type === 'win' ? 'text-green-500' : 'text-red-500'} font-medium`}>
                            {game.pnl}
                          </span>
                        </div>
                      </div>
                  ...
                  {/* Empty placeholders to keep height stable for exactly 6 items */}
                  {Array.from({ length: Math.max(0, PAGE_SIZE - pagedGameHistory.length) }).map((_, i) => (
                    <div
                      key={`placeholder-${i}`}
                      className="border border-transparent/30 rounded-lg p-3 opacity-0"
                    >
                      &nbsp;
                    </div>
                  ))}
                ...
                <div className="mt-4 flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div className="text-sm tabular-nums w-16 text-center">
